   ...
   public function index()
{
    ...
    // Redirigir si no tiene permiso de ver dashboard
    if (!$user->hasPermissionTo('ver dashboard')) {
        return match(true) {
            $user->hasRole('pos')       => redirect()->route('admin.pos.create'),
            $user->hasRole('logistica') => redirect()->route('admin.dispatches.index'),
            $user->hasRole('ventas')    => redirect()->route('admin.sales-orders.index'),
            $user->hasRole('cxc')       => redirect()->route('admin.ar.index'),
            $user->hasRole('cajero')    => redirect()->route('admin.cash.index'),
    ...
    // ── Ventas del día ────────────────────────────────────────────────────
    $soHoy   = (float) SalesOrder::whereDate('updated_at', $today)->where('status', 'ENTREGADO')->sum('total');
    $posHoy  = (float) Sale::whereDate('fecha', $today)->sum('total');
    $ventasHoy = $soHoy + $posHoy;

    $soAyer  = (float) SalesOrder::whereDate('updated_at', $yesterday)->where('status', 'ENTREGADO')->sum('total');
