...
>
    ...
    <x-wire-card class="mb-4">
        <div class="flex items-center gap-4">
            <div class="w-12 h-12 rounded-full bg-indigo-100 flex items-center justify-center">
                <i class="fa-solid fa-user text-indigo-600 text-lg"></i>
            </div>
            <div>
                ...
                <div class="text-sm text-gray-500">
                    {{ now()->isoFormat('dddd D [de] MMMM [de] YYYY — HH:mm') }}
                    · Rol:
                    @foreach(auth()->user()->roles as $r)
                        <span class="px-1.5 py-0.5 rounded text-xs font-medium bg-indigo-100 text-indigo-700">
                            {{ ucfirst($r->name) }}
                        </span>
                    @endforeach
                </div>
        ...
        <div class="mt-4 flex flex-wrap gap-2">
            @can('usar pos')
            <a href="{{ route('admin.pos.create') }}"
               class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-indigo-600 text-white text-sm hover:bg-indigo-700">
                <i class="fa-solid fa-barcode"></i> Ir al POS
            </a>
            @endcan
            @can('ver cajas')
            <a href="{{ route('admin.cash.index') }}"
               class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-emerald-600 text-white text-sm hover:bg-emerald-700">
                <i class="fa-solid fa-cash-register"></i> Cajas
            </a>
            @endcan
            @can('ver despachos')
            <a href="{{ route('admin.dispatches.index') }}"
               class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-amber-500 text-white text-sm hover:bg-amber-600">
                <i class="fa-solid fa-truck-fast"></i> Despachos
            </a>
            @endcan
            @can('ver pedidos')
            <a href="{{ route('admin.sales-orders.index') }}"
               class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-violet-600 text-white text-sm hover:bg-violet-700">
            ...
            <a href="{{ route('admin.ar.index') }}"
               class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-blue-600 text-white text-sm hover:bg-blue-700">
                <i class="fa-solid fa-piggy-bank"></i> CxC
            </a>
            @endcan
        </div>
    </x-wire-card>
    ...
    {{-- ── KPIs y gráficas solo para admin ── --}}
    @if($isAdmin)
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <x-wire-card>
            <div class="flex items-start justify-between">
                <div>
                    ...
                    <div class="mt-1 text-2xl font-bold text-gray-800">${{ number_format($ventasHoy, 2) }}</div>
                    <div class="mt-1 text-xs text-gray-400">Ayer: ${{ number_format($ventasAyer, 2) }}</div>
                </div>
                <span class="px-2 py-0.5 rounded-full text-xs font-medium mt-1 {{ $variacion >= 0 ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700' }}">
        ...
        <x-wire-card>
            <div class="flex items-start justify-between">
                <div>
                    ...
                    <div class="mt-1 text-2xl font-bold text-gray-800">{{ number_format($pedidosHoy) }}</div>
                    <div class="mt-1 text-xs text-gray-400">Ayer: {{ number_format($pedidosAyer) }}</div>
                </div>
                @php $varPedidos = $pedidosAyer > 0 ? round((($pedidosHoy-$pedidosAyer)/$pedidosAyer)*100,1) : 0; @endphp
                <span class="px-2 py-0.5 rounded-full text-xs font-medium mt-1 {{ $varPedidos >= 0 ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700' }}">
        ...
        <x-wire-card>
            <div>
                ...
                <a href="{{ route('admin.ar.index') }}" class="mt-1 text-xs text-indigo-500 hover:underline">Ver detalle →</a>
            </div>
        </x-wire-card>
        ...
        <x-wire-card>
            <div>
                ...
                <div class="mt-1 text-2xl font-bold text-violet-600">{{ $pedidosPorEstado['EN_RUTA'] ?? 0 }}</div>
                <a href="{{ route('admin.sales-orders.index') }}" class="mt-1 text-xs text-indigo-500 hover:underline">Ver pedidos →</a>
            </div>
        </x-wire-card>
    </div>
            ...
            <div class="space-y-2">
                @foreach($estados as $key => $meta)
                    ...
                    <div class="flex items-center justify-between">
                        <span class="px-2 py-0.5 rounded-full text-xs {{ $meta['color'] }}">{{ $meta['label'] }}</span>
    ...
    <x-wire-card class="mt-4">
        <div class="flex items-center justify-between mb-3">
            ...
            <a href="{{ route('admin.sales-orders.index') }}" class="text-xs text-indigo-500 hover:underline">Ver todos →</a>
        </div>
                        ...
                        <tr class="hover:bg-gray-50">
                            <td class="p-2">
                                <a href="{{ route('admin.sales-orders.edit', $p) }}"
                                   class="text-indigo-600 hover:underline font-mono text-xs">{{ $p->folio }}</a>
                            </td>
                            <td class="p-2 text-gray-700">{{ $p->client?->nombre ?? '—' }}</td>
                            <td class="p-2 text-right font-medium">${{ number_format($p->total, 2) }}</td>
                            <td class="p-2 text-center">
                                <span class="px-2 py-0.5 rounded-full text-xs {{ $sc }}">{{ $p->status }}</span>
                            </td>
    ...
    (function(){
        ...
        new Chart(ctx, {
            ...
            data: {
                labels,
                datasets: [
                    { label: 'Pedidos', data: so,  backgroundColor: 'rgba(99,102,241,0.7)', borderRadius: 4, stack: 'ventas' },
                    { label: 'POS',     data: pos, backgroundColor: 'rgba(16,185,129,0.7)', borderRadius: 4, stack: 'ventas' },
                ],
            },
            options: {
                responsive: true,
                plugins: {
                    ...
                    tooltip: { callbacks: { label: ctx => ' $' + Number(ctx.raw).toLocaleString('es-MX', {minimumFractionDigits:2}) } }
                },
                scales: {
                    x: { stacked: true, grid: { display: false } },
                    y: { stacked: true, ticks: { callback: v => '$' + Number(v).toLocaleString('es-MX') } },
                },
            },
        });
    })();
