    ...
    <x-wire-card>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
                ...
                <tbody id="orders-tbody" class="divide-y divide-gray-200 dark:divide-gray-700">
                    @foreach($orders as $order)
                    <tr
                        data-search="{{ strtolower($order->folio . ' ' . $order->provider?->nombre . ' ' . $order->warehouse?->nombre) }}"
                        data-status="{{ $order->status }}"
                    >
                        <td class="px-4 py-3 text-gray-500 dark:text-gray-400">{{ $order->id }}</td>
                        <td class="px-4 py-3 font-medium text-gray-900 dark:text-gray-100">{{ $order->folio }}</td>
                        <td class="px-4 py-3 text-gray-600 dark:text-gray-400">{{ $order->provider?->nombre ?? '—' }}</td>
                        <td class="px-4 py-3 text-gray-600 dark:text-gray-400">{{ $order->warehouse?->nombre ?? '—' }}</td>
                        <td class="px-4 py-3 text-gray-600 dark:text-gray-400">{{ $order->fecha }}</td>
                        <td class="px-4 py-3">
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-2 flex-wrap">
                                ...
                                @if($order->status === 'draft')
                                    <form action="{{ route('admin.purchase-orders.approve', $order) }}" method="POST" class="inline">
                                        @csrf
                                        <button type="submit"
                                                class="inline-flex items-center px-2 py-1 text-xs rounded-md bg-green-600 text-white hover:bg-green-700">
                                            Aprobar
                                        </button>
                                    </form>
                                    <form action="{{ route('admin.purchase-orders.destroy', $order) }}" method="POST" class="delete-form inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                ...
                                @if(in_array($order->status, ['draft','approved']))
                                    <form action="{{ route('admin.purchase-orders.cancel', $order) }}" method="POST" class="inline">
                                        @csrf
                                        <button type="submit"
                                ...
                                @if($order->status === 'approved')
                                    <a href="{{ route('admin.purchases.create', ['purchase_order_id' => $order->id]) }}"
                                       class="inline-flex items-center px-2 py-1 text-xs rounded-md bg-violet-600 text-white hover:bg-violet-700">
                                        Crear compra
                                    </a>
                                @endif
                            </div>
    ...
    (function () {
        ...
        function renderPage() {
            ...
            paginationInfo.textContent = total > 0
                ? `Mostrando ${start + 1}–${Math.min(end, total)} de ${total} órdenes`
                : '';
